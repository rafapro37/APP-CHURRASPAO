...
export function Badge({ children, className }: { children: React.ReactNode; className?: string }) {
  return <span className={cn("inline-flex items-center rounded-full bg-brand px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide text-white", className)}>{children}</span>;
}

function NoPhoto() {
  return (
    <div className="flex h-full w-full items-center justify-center bg-[#050505] text-[11px] font-medium text-muted-foreground">
      Sem foto
    </div>
  );
}
    ...
    <div className="group flex min-h-[236px] flex-col overflow-hidden rounded-2xl border border-border bg-card transition-all duration-200 hover:border-brand/50 hover:shadow-[0_8px_30px_rgba(217,101,8,0.16)] sm:min-h-[292px]">
      ...
      <div className="flex flex-1 flex-col gap-1.5 p-2.5 sm:p-3.5">
        <Link href={`/produto/${product.id}`}>
          <h3 className="line-clamp-2 font-display text-sm font-semibold leading-tight sm:text-base">{product.name}</h3>
          {product.shortDescription && <p className="line-clamp-2 text-[11px] leading-snug text-muted-foreground sm:text-xs">{product.shortDescription}</p>}
        </Link>
        <div className="mt-auto flex items-end justify-between gap-2 pt-2">
          <div className="min-w-0">
            {product.price <= 0 ? (
              ...
              <>
                {showStrike && <span className="block text-[10px] text-muted-foreground line-through">{formatBRL(product.price)}</span>}
                <span className={cn("font-display text-base font-bold sm:text-lg", promo ? "text-brand-bright" : "text-foreground")}>{formatBRL(promo ?? product.price)}</span>
              </>
            )}
          </div>
