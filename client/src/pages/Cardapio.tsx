...
export default function Cardapio() {
  ...
  return (
    <AppLayout>
      <section className="sticky top-[73px] z-20 border-b border-border bg-background/95 pb-3 pt-3 backdrop-blur">
        <div className="mx-auto w-full max-w-[430px] px-4 md:max-w-5xl">
          <div className="flex items-center gap-3">
            <Link href="/" className="-ml-2 flex h-10 w-10 items-center justify-center rounded-full hover:bg-card">
              <ArrowLeft className="h-5 w-5" />
            </Link>
            <img src={logo} alt="CHURRASPÃO E CIA" className="h-10 w-10 rounded-full object-cover ring-1 ring-brand/60" />
            <div className="min-w-0 flex-1">
              <h1 className="font-display text-lg font-bold leading-tight">Cardápio</h1>
              <p className="truncate text-[11px] text-muted-foreground">{BRAND.name}</p>
            </div>
            <Link href="/carrinho" className="relative flex h-10 w-10 items-center justify-center rounded-full border border-border bg-card">
          ...
          <div className="relative mt-3">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar produto ou bebida..." className="w-full rounded-2xl border border-border bg-card py-3 pl-10 pr-4 text-sm outline-none transition-shadow placeholder:text-muted-foreground focus:ring-2 focus:ring-brand/60" />
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto w-full max-w-[430px] px-4 pt-4 md:max-w-5xl">
        <div className="-mx-4 flex gap-2 overflow-x-auto px-4 pb-2" style={{ scrollbarWidth: "none" }}>
          <button onClick={() => setSelectedCat(null)} className={cn("btn-press flex h-10 shrink-0 items-center gap-1.5 rounded-full border px-4 text-sm font-semibold transition-colors", selectedCat === null ? "border-brand bg-brand text-white" : "border-border bg-card text-muted-foreground hover:text-foreground")}>
            <UtensilsCrossed className="h-4 w-4" /> Tudo
          </button>
          {categories.map((category) => (
            <button key={category.id} onClick={() => setSelectedCat(category.id)} className={cn("btn-press flex h-10 shrink-0 items-center rounded-full border px-4 text-sm font-semibold transition-colors", selectedCat === category.id ? "border-brand bg-brand text-white" : "border-border bg-card text-muted-foreground hover:text-foreground")}>
              {category.name}
            </button>
          ))}
        </div>
      ...
      <section className="mx-auto w-full max-w-[430px] px-4 pt-2 md:max-w-5xl">
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-3 pb-8 md:grid-cols-4 md:gap-4">
